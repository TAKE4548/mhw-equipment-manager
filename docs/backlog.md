...
- **Problem**: 現在導入されているアイコンアセットについて、視認性、統一感、およびゲーム（MHW）の世界観との整合性を再評価する必要がある。
- **Acceptance Criteria**:
    - [x] 全ての主要アイコンが v15 HUD デザインガイドラインに適合。
    - [x] 「アイコン・オーバーレイ（複合アイコン）」デザインによる視認性の向上を達成。
...
- **Acceptance Criteria**:
    - [x] 主要なアイコンアセット（武器・属性・スキル・スロット）が SVG 形式に移行済み。
    - [x] CSS `mask-image` 方式を採用し、動的なカラーリング（HUD配色への追従）が可能。
    - [x] 武器と属性を統合した複合アイコン実装（Walkthrough: [walkthrough.md](file:///C:/Users/audih/.gemini/antigravity/brain/f094922c-f576-4b22-a63a-b05c84a3a4db/walkthrough.md)）

### REQ-035: 複合アイコン化に伴うカード内情報の再配置と密度最適化
- **Status**: done (2026-04-11)
- **Type**: ui-ux
- **Priority**: mid
- **Source**: REQ-034 完了後のフィードバック
- **Problem**: 武器種と属性を一つのアイコンに統合したことで、従来の属性情報用カラム（`v15-col-spec`）に余白が生まれ、情報の密度をさらに高める余地が生じている。
- **Requirement**: 複合アイコン化によって空いたスペースを活用し、スキル、ボーナス、あるいはカスタム識別名などの重要な情報をより大きく、あるいは多く表示できるようにレイアウトを再設計する。
- **Acceptance Criteria**:
    - [x] `v15-col-anchor` と `v15-col-spec` の幅・配置の見直しが完了している。
    - [x] スキル名やボーナス情報の表示領域が拡大され、視認性が向上している。
    - [x] 情報の優先順位に基づいた新しい v15 HUD 用の各カラム比率が定義されている。
    - [x] 武器名と属性情報を ID Cluster の 2 行に集約。
