...
- **Acceptance Criteria**:
    - [x] `update_talisman` 等の内部で `validate_talisman` を呼び出し、失敗時は処理を遮断する。
    - [x] 武器・強化トラッカーの登録時にも妥当性チェックを行う。
    - [x] バリデーションエラーを UI に通知できる仕組み（例外処理）を導入。
...
### REQ-036: 復元強化厳選カードにおける「生産ボーナス」の配置変更
- **Status**: done (2026-04-11)
- **Current Step**: none
- **Type**: ui-ux
- **Priority**: mid
- **Source**: "復元強化厳選のページで、カード内の生産ボーナス列を復元ボーナス比較列の左側にしたい"
- **Problem**: 復元強化厳選画面において、武器の恒常的なステータスである「生産ボーナス」が「復元ボーナス比較」の右側に配置されており、情報の読み取り順序として不自然に感じられる場合がある。
- **Acceptance Criteria**:
    - 復元強化厳選（`mode="reinforcement"`）のカードにおいて、「生産ボーナス（🛠️）」列が「復元ボーナス比較（✨/➡️）」列の左側に配置されていること。
    - 他の画面（装備ボックス等）の標準レイアウトには影響を与えないこと。

### REQ-037: 武器カードの復元ボーナス略称表記 (Abbreviated Restoration Bonus)
- **Status**: ready
- **Type**: enhancement
- **Priority**: mid
- **Source**: "所持武器一覧のカード内での復元ボーナスの表記を略称表記にしたい"
- **Problem**: 所持武器一覧のカードにおいて、復元ボーナスの名称が長いため、限られた表示領域内で情報が圧迫され、視認性が低下している。
- **Acceptance Criteria**:
    - 所持武器一覧のカード内において、復元ボーナスが適切に省略・略称化して表示されること（例: "攻撃力強化" -> "攻撃" 等）。
    - 略称化により、カード内の情報密度とレイアウトの余裕が向上していること。
    - 略称化のルールが明確に定義されていること。

### REQ-038: ページ遷移時の読み込みパフォーマンス改善 (Page Navigation Performance)
- **Status**: ready
- **Type**: performance
- **Priority**: high
- **Source**: "各ページ切り替え後の読み込みが遅くなった気がする"
- **Problem**: ページ切り替え（ナビゲーション）時に無視できない描画遅延やロード時間が発生しており、ユーザーの作業のテンポを阻害している。
- **Acceptance Criteria**:
    - 各ページの読み込み速度（特に遷移時）が改善され、体感的にストレスのない速度になること。
    - 重い処理のキャッシュ化、巨大なレンダリング要素の遅延評価、または不要な再描画の抑制などの対処が行われていること。
    - パフォーマンス悪化の根本原因（ボトルネック）が特定・解消されていること。

### REQ-039: 激化タイプの視覚的識別性の向上
- **Status**: ready
- **Type**: ui-ux
- **Priority**: mid
- **Source**: "武器のカードで激化タイプによって、エッジの色とかで直観的に見分けつくようにしたい\nモンハンワイルズの巨戟アーティア武器の激化タイプ(攻撃激化・会心激化・属性激化)のこと\n今のカードデザイン的に、激化タイプの表示が控えめでぱっと見で認識できない。厳選のベースとかになるので、同じスキル構成とかだと重複登録しちゃったのか、しっかり見ないとわからなくて、検索性がわるいから"
- **Problem**: 武器カードにおいて、厳選のベースとなる「激化タイプ（攻撃・会心・属性）」の表示が控えめであり、テキストを読まないと判別しづらい。これにより、重複登録の確認や複数武器の比較が困難になり、検索性が低下している。
- **Requirement**: 武器カードをパッと見ただけでどの激化タイプであるかが直感的に識別できるように、カードの視覚的デザインを改修する。
- **Acceptance Criteria**:
    - 武器カード上で激化タイプ（攻撃・会心・属性）の違いが視覚的（ボーダー色やアイコンの強調など）に明確に区別できること。
    - スキル構成が同一であっても、激化タイプが異なれば一目で別物と認識できること。
    - v15 HUD の全体デザインと親和性のある形で表現されること。

### REQ-040: アプリ全域のコンポーネント余白（クリアランス）統一
- **Status**: ready
- **Type**: ui-ux
- **Priority**: mid
- **Source**: "各ページの登録とフィルタの間のクリアランスが不自然に開いてる\nスッキリさせたいというだけではある。護石のページのカード間だけ他ページとクリアランスが違うのも気になる。アプリ全体としての類似デザイン箇所の統一感のなさが気持ち悪い"
- **Problem**: ページ上部の操作パネル（登録用・フィルタ用エキスパンダー等）の間隔や、リスト表示部分のカード間の余白スタイルがページごとにばらついており、システム全体の統一感が損なわれている。不自然な空白によりレイアウトに違和感を生じさせている。
- **Requirement**: アプリケーション全体を通じて、コンポーネント間（特にエキスパンダー間やリストアイテム間）の余白（マージン/パディング）設定を監査し、統一された基準（Lean UI等）に基づいて最適なクリアランスに揃える。
- **Acceptance Criteria**:
    - 登録・フィルタ等のエキスパンダー領域について、全ページで垂直余白が統一されている。
    - 護石管理ページにおけるカード表示間隔など、リスト領域の余白が他ページと同じ基準で統一されている。
    - 不要な空白がなくなり、全体のUI体験に一貫性が確保されている。
