...
- **Acceptance Criteria**:
    - 以下の「下位互換」ルールに基づき、不要な護石を自動抽出できる：
        1. スキル種類が同一で、各レベルが対象以下かつ、スロット構成も対象以下（かつ、いずれかの要素が対象より劣る）。
        2. スロットが優秀であっても、スキルを含めた総合的な「装備の可能性」で他方の護石に完全に内包されるもの。
    - 抽出された「整理候補」を一覧表示し、削除やロック解除を一括または個別で行えるUI。
- **Note**: 上位互換の判定アルゴリズム（特にスロットの重み付けや複数スキルの組み合わせ）について、設計時に詳細な検討が必要。
...
- **Note**: `src/components/cards.py` の `render_slim_card` における `hud` モードと `reinforcement` モードの共通化、または `hud` モード内での個別対応を検討する。
