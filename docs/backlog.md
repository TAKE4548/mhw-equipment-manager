...
- **Acceptance criteria**:
    - [x] 各リストページで、アイテム間の隙間が視覚的に一定（ガタつきがない）であること。
    - [x] Streamlit のアップデートや各ブラウザの `:has` サポート状況に左右されない堅牢な実装であること。
- **Concerns**: 全体の Gap を 8px に縮小したため、将来的に広い余白を必要とするページ構成では個別対応が必要になる可能性がある。
