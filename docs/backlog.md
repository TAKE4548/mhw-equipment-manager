...
### REQ-028: リストアイテム間の表示間隔の安定化と制約回避
- **Status**: new
- **Type**: defect
- **Priority**: low
- **Problem**: Streamlit の `st.container` や `st.columns` が生成する親要素 (`stVerticalBlock`) のデフォルト `gap` 設定が強力であり、CSS による子要素側からの余白制御 (`margin-bottom`) が意図通りに反映されない、あるいは環境によって挙動が不安定になる。
- **Requirement**: CSS ハック（`:has` セレクタ等）に頼らず、リストアイテム間の適切なクリアランス（現状は 8px を目標）を安定して確保する手法を確立する。
- **Acceptance criteria**:
    - 各リストページで、アイテム間の隙間が視覚的に一定（ガタつきがない）であること。
    - Streamlit のアップデートや各ブラウザの `:has` サポート状況に左右されない堅牢な実装であること。
