...
def test_filter_equipment_restoration_and_logic():
    df = pd.DataFrame([
        {
            ...
            "enhancement_type": "Attack",
            "p_bonus_1": "なし", "p_bonus_2": "なし", "p_bonus_3": "なし",
            "rest_1_type": "攻撃強化", "rest_1_level": "Ⅱ",
    ...
    # Single filter
    res = filter_equipment(df, restoration_bonuses=["攻撃強化 [Ⅱ]"])
    assert len(res) == 2
    
    # AND filter (攻撃Ⅱ & 会心Ⅲ)
    res = filter_equipment(df, restoration_bonuses=["攻撃強化 [Ⅱ]", "会心率強化 [Ⅲ]"])
    assert len(res) == 1
    assert res.iloc[0]['id'] == "1"
    
    # AND filter (攻撃Ⅱ & 属性強化)
    res = filter_equipment(df, restoration_bonuses=["攻撃強化 [Ⅱ]", "属性強化"])
    assert len(res) == 1
    assert res.iloc[0]['id'] == "2"
